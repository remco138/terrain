		...
		for(int level = 0; level < lodDistances.size(); level++)
		{
			for(int i = 0; i < children.size(); i++)
			{
				if((children[i]->boundingBox.getCenter() - viewpoint).length() < lodDistances[level])
				{
					...
					std::vector<float> vertecis = streamSource(bbox.getMinCorner().removeY(), bbox.getSize().removeY(), pow(2, level - lodDistances.size()));
					...
					std::vector<VAO> vao(4);
					//upload to GPU, multiple index buffers will do their pointing to a single vertex buffer
					for(int k = 0; k < ibo.size(); k++) { vao[k].create(); vao[k].addVbo(vbo); vao[k].addIBO(ibo[k]); vao[k].create(); }
